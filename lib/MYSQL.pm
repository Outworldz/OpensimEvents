...
# "DSN=$DSN;UID=$login;PWD=$password;"
sub new
{
	shift;
	my $connect_line= shift;
	my $entire_line = $connect_line;
	...
	$self->{'DBI_DBH'}=DBI->connect(
			...
			$self->{'DBI_USER'},
			$self->{'DBI_PASSWORD'},
			{ RaiseError => 0, AutoCommit => 1,LongReadLen => 256000,LongTruncOk=>1, PrintError=>0,ChopBlanks => 1});
...
# so if the data read back is > 4000, do not throw an error.
	...
	if (! $self->{'DBI_DBH'})
	{
		warn "Error($DBI::err) : $DBI::errstr\n" unless (length($ENV{REMOTE_ADDR}) 	> 0 ); # supress warns in browsers

		$self->DBIODBCErrorLog("->New($entire_line) Error= Error($DBI::err) : $DBI::errstr");
...
sub Sql
{
	...
	$self->DBIODBCErrorLog("->Sql($SQL_statment) Error= Error($DBI::err) : $DBI::errstr") if $DBI::err;
...
sub DBIODBCErrorLog
{
	...
	my $log = $self->{'Logging'};  # this log always enabled

	if (!$log)
	{
		our $LOG = $main::LOG;
		if(open(LOGFILE,">>$LOG/SQL_ErrorLog.txt"))
		{
			my $Date;
			$Date = GetDate();
			$Date = GetDate();
...
sub DBIODBCLog
{
	my $log = 0; # set this to 1 to log all Sql
	if ($log > 0)
	{
		my $msg = shift;
		our $LOG = $main::LOG;
		if(open(LOGFILE,">>$LOG/sqllog.txt"))
		{
			my $Date;
			$Date = GetDate();
			$Date =~ s/\//-/g;
...
sub Error
{
 	...
 	if ($self->{'DBI_ERR'} ne '')
 	{
		$self->DBIODBCErrorLog("Warning from DBIODBC.pm for ($self->{'DBI_SQL_STATMENT'}) of $self->{'DBI_ERRSTR'}");
...
# [FKB] must call finish method before destroying with later DBI module
	my $sth=$self->{'DBI_STH'};
	$sth->finish() if $self->{'DBI_SQL_STATMENT'}; #Cannot  call finish method if we have no sql statement
...
sub Execute
{
	...
	$self->DBIODBCErrorLog("->Sql($self->{'DBI_SQL_STATMENT'}) Error= Error($DBI::err) : $DBI::errstr") if $DBI::err;
